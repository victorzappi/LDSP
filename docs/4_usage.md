...
- **Windows users:** you can use the version of the main script called *[ldsp.bat](../scripts/ldsp.bat)* (batch script version), either from a Command Prompt or from a PowerShell—we recommend the latter. However, if you have a Unix-like environment installed, like *Cygwin* or *MinGW*, you should be able to use *ldsp.sh* too. If you installed Git as a [dependency](1_dependencies.md), you can use *Git Bash* to run *ldsp.sh*!
...
#### Windows
```console
.\scripts\ldsp.bat configure --configuration=".\phones\LG\Nexus 4 (mako)" --version=5.1.1 --project=.\examples\Fundamentals\sine
```

> **Note:** Use quotes around paths that contain spaces or special characters (e.g., parentheses).

This step will set up the building environment and will generate a persistent configuration file, hence it needs to be repeated only if we switch to a different phone or project!
...
If everything goes right, a new bin file called *ldsp* will appear in the newly created *LDSP/build/bin* folder!
...
- **Run remotely over WiFi**. This is a more advanced option that may not work on very old phones/Android versions. 
    ...
    ABD allows for untethered connection via TCP when computer and phone are connected to the same WiFi network (including any hostspot generated by the phone itself). You can find instructions on how to set up this connection here:
